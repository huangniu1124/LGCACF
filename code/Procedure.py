...
def BPR_train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    users = torch.tensor(np.array(S[:, 0])).long()
    ...
    for (batch_i,
        ...
        cri, reg_l = bpr.stageOne(batch_users, batch_pos, batch_neg)
        aver_loss += cri
        reg_loss += reg_l
    ...
    return f"[BPR[aver loss{aver_loss:.3e}, {reg_loss:.3e}]"
...
def Test(dataset, Recmodel, epoch, w=None, multicore=1):
    u_batch_size = world.config['test_u_batch_size']    # 100
    dataset: utils.BasicDataset
    testDict: dict = dataset.testDict
    # Recmodel: model.LightGCN
    Recmodel: model.MultiLGCN
